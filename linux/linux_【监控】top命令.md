...
##  按照 RSS 内存大小对进程进行排序
如果您不能使用ps、smem或其他第三方命令，并且需要按照 RSS 内存大小对进程进行排序，并以 GB 为单位显示内存占用大小和进程名
...
# 遍历进程目录，获取每个进程的 RSS 内存信息
for dir in "${process_dirs[@]}"; do
  pid=$(basename "$dir")
  if [[ $pid =~ ^[0-9]+$ ]]; then
    rss=$(awk '/VmRSS/ {size=$2/1024/1024; print size}' "${dir}/status" 2>/dev/null)
    if [[ -n $rss ]]; then
      cmd=$(tr -d '\0' <"${dir}/cmdline")
