...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/96298046051fbb161b41c12012d0b212.jpeg#pic_center)

## 解压

```bash
ubectl get secret  -n prometheus  prometheus-prometheus-kube-prometheus-prometheus -oyaml
apiVersion: v1
data:
...
例如修改参数例如：scrape_timeout: 30s改为60s
...
```bash
kubectl get secret  -n prometheus  prometheus-prometheus-kube-prometheus-prometheus
```
